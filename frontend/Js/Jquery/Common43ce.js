...
// 한국 운송장 조회
function fnKrDlvrSearch(IvcComCd, IvcNo) {
	var Url = '';

	if ( IvcNo == '' ) {
		alert('운송장번호를 입력하십시오.'); return;
	} else {
		window.open('/Front/Main/TrkSearchPop_S.asp?NATION_TYPE=KR&IVC_COM_CD='+IvcComCd+'&TRACKING_NO='+IvcNo, 'KrDlvrSearch', 800, 800);
	}
}
...
//앱 트래킹 및 오더 html 가져오기 --
// 트래킹 가져오기 내용
function getTraking(_targetUrl) {
	try {
		 // For Android
		window.JavascriptInterface.getTraking(_targetUrl);
	} catch (e) {
		 // For iOS
		var _methodName = "getTraking";
		var _url  = "JavascriptInterface:";
		    _url += "//" + _methodName
		    _url += "//" + _targetUrl
...
// 트래킹 가져오기 시 로그인 페이지 이동
function loginForGetTraking(_targetUrl) {
	alert("스크립호출");
	try {
		 // For Android
		window.JavascriptInterface.loginForGetTraking(_targetUrl);
	} catch (e) {
		 // For iOS
		var _methodName = "loginForGetTraking";
		var _url  = "JavascriptInterface:";
		    _url += "//" + _methodName
		    _url += "//" + _targetUrl
		document.location = _url;//_targetUrl;//_url;
	}
}

// 오더내역복사
function copyOrder(_targetUrl) {
	try {
		 // For Android
		window.JavascriptInterface.copyOrder(_targetUrl);
	} catch (e) {
		 // For iOS
		var _methodName = "copyOrder";
		var _url  = "JavascriptInterface:";
		    _url += "//" + _methodName
		    _url += "//" + _targetUrl
		document.location = _url;
...
// 오더내역복사 시 로그인 페이지 이동
function loginForCopyOrder(_targetUrl) {
	try {
		 // For Android
		window.JavascriptInterface.loginForCopyOrder(_targetUrl);
	} catch (e) {
		 // For iOS
		var _methodName = "loginForCopyOrder";
		var _url  = "JavascriptInterface:";
		    _url += "//" + _methodName
		    _url += "//" + _targetUrl
		document.location = _url;
